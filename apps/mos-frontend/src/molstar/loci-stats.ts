...
import { ChainIndex } from "molstar/lib/mol-model/structure"
...
export namespace Stats {
    ...
    function handleElement(stats: Stats, structure: Structure, element: Loci['elements'][0]) {
        ...
        if (size === elements.length) {
            stats.unitCount += 1;
            {
              const loc = Location.create(void 0);
              Location.set(loc, structure, unit, elements[OrderedSet.start(indices)]);
              stats.unitLocs.push(loc);
            }
